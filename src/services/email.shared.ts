...
export type DriverEmailType = 'approval' | 'rejection';
...
export interface NewDriverAdminEmailPayload {
  adminName?: string;
  driverName: string;
  driverEmail: string;
  vehicleType: string;
...
export const buildDriverActionUrl = (email: string): string =>
  `${APP_URL}/auth?role=driver&email=${encodeURIComponent(email)}`;
...
export const driverApprovalHtml = (email: string, driverName: string): string => `
  <!DOCTYPE html>
  <html>
  <head>
    <meta charset="UTF-8">
  ...
  <body style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: white; padding: 40px 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
      ...
      <p style="font-size: 16px; margin-bottom: 20px;">
        Excellente nouvelle! Votre profil chauffeur <strong style="color: #16a34a;">${driverName}</strong> a été approuvé par notre équipe.
      </p>

      <div style="background: #f0fdf4; border-left: 4px solid #16a34a; padding: 16px; margin: 30px 0; border-radius: 6px;">
        <p style="margin: 0; font-size: 14px; color: #166534;">
          Vous pouvez maintenant créer votre compte chauffeur et commencer à accepter des livraisons sur la plateforme Ouyaboung.
        </p>
      </div>
      ...
      <div style="text-align: center; margin: 40px 0;">
        <a href="${buildDriverActionUrl(email)}"
           style="display: inline-block; background: #16a34a; color: white; padding: 16px 32px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px; box-shadow: 0 4px 6px rgba(22, 163, 74, 0.2);">
...
export const driverRejectionHtml = (driverName: string, reason?: string): string => `
  <!DOCTYPE html>
  <html>
  <head>
    <meta charset="UTF-8">
  ...
  <body style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: white; padding: 40px 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
      ...
      <p style="font-size: 16px; margin-bottom: 20px;">
        Nous avons examiné votre demande d'inscription pour <strong>${driverName}</strong>.
      </p>
...
export const newDriverAdminHtml = (payload: NewDriverAdminEmailPayload): string => `
  <!DOCTYPE html>
  <html>
  <head>
    <meta charset="UTF-8">
  ...
  <body style="font-family: Arial, sans-serif; color: #1f2937; line-height: 1.5;">
    <div style="max-width: 620px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden;">
      <div style="background: #111827; color: #ffffff; padding: 20px;">
        <h1 style="margin: 0; font-size: 20px;">Nouveau chauffeur en attente</h1>
      </div>
      <div style="padding: 20px;">
        <p>Bonjour${payload.adminName ? ` ${payload.adminName}` : ""},</p>
        <p>Un nouveau chauffeur s'est inscrit et attend une validation administrateur.</p>
        <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
          <tr><td style="padding: 6px 0; color: #6b7280;">Nom</td><td style="padding: 6px 0;"><strong>${payload.driverName}</strong></td></tr>
          <tr><td style="padding: 6px 0; color: #6b7280;">Email</td><td style="padding: 6px 0;">${payload.driverEmail}</td></tr>
          <tr><td style="padding: 6px 0; color: #6b7280;">Véhicule</td><td style="padding: 6px 0;">${payload.vehicleType}</td></tr>
          <tr><td style="padding: 6px 0; color: #6b7280;">Ville</td><td style="padding: 6px 0;">${payload.city}</td></tr>
          <tr><td style="padding: 6px 0; color: #6b7280;">Date création</td><td style="padding: 6px 0;">${payload.createdAt}</td></tr>
        </table>
        <p>
