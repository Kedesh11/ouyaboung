/**
 * Edge Function: Airtel/Moov Callback (Structure Unifiée Q-Gabon)
 * 
...
serve(async (req) => {
    ...
    try {
        ...
        console.log('[Airtel Callback] Logged to webhook_logs:', logId.data?.id)
        ...
        if (!reference) {
            console.error('[Airtel Callback] Missing merchantReferenceId')
            return new Response(
        ...
        if (txError || !transaction) {
            console.error('[Airtel Callback] Transaction not found:', reference)
        ...
        console.log('[Airtel Callback] Transaction found:', transaction.id)
        ...
        if (transaction.status === 'SUCCESS' && status === 'SUCCESS') {
            console.log('[Airtel Callback] Transaction already processed as SUCCESS')
            await supabaseClient.from('webhook_logs').update({
                processed: true,
        ...
        if (updateError) {
            console.error('[Airtel Callback] Update error:', updateError)
            await supabaseClient.from('webhook_logs').update({
                processed: false,
        ...
        console.log('[Airtel Callback] Transaction updated to:', finalStatus)
        ...
        if (finalStatus === 'SUCCESS') {
            ...
            if (orderError) {
                console.error('[Airtel Callback] Order update error:', orderError)
            } else {
                console.log('[Airtel Callback] Order confirmed:', transaction.order_id, 'QR Code:', pickupCode)
            }
        }
        ...
        if (finalStatus === 'SUCCESS' || finalStatus === 'FAILED') {
            ...
            if (notificationError) {
                console.error('[Airtel Callback] Notification error:', notificationError)
            }
        }
